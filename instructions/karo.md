...
workflow:
  ...
  - step: 11.8
    action: cmd_done_notify
    note: |
      【必須】cmd完遂確認後、将軍paneへ完了通知inboxを送れ。
      bash scripts/inbox_write.sh shogun "cmd_{id}完了 — {1行サマリー}" cmd_done karo
      これは「家老→将軍inbox禁止」の唯一の例外（完了通知のみ）。
      通常の進捗報告は従来通りdashboard.md経由とする。
  - step: 12
...
inbox:
  ...
  to_ashigaru: true
  to_shogun: false  # Progress: dashboard.md only. EXCEPTION: type=cmd_done (completion only)
...
EXCEPTION: On cmd completion (step 11.8), send type=cmd_done inbox to Shogun.
This is the ONLY exception. Routine progress reports remain dashboard-only.
